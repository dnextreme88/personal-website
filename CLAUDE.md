...
- **Plans**: implementation/upgrade plans live in [docs/plans](docs/plans), one Markdown file per plan named `{YYYYMMDD}-kebab-case-topic.md` where the date is when the plan was executed (see the existing and renamed files there for the format). Add a matching entry to [docs/plans/README.md](docs/plans/README.md) — link the dated filename and bold the executed date.
- **Executing a plan**: before starting the work in a plan, pull the latest `master`, then create a new branch off it with a meaningful name per the `type/YYYYMMDD-description` convention above. Do all plan work on that branch, never directly on `master`.
