...
<button
    ...
    {{ $attributes->except('type')->merge(['class' => 'btn-submit group relative inline-block leading-none cursor-pointer']) }}
>
    <span
    ...
    >
        ...
        <span aria-hidden="true" class="absolute inset-0 bg-green-500"></span>

        {{-- Fill — inset by the 2px border width, re-clipped to the same shape. --}}
        <span aria-hidden="true" class="absolute inset-0.5 bg-green-800 dark:bg-green-200 {{ $surface_class }}" style="clip-path: {{ $clip_path }};"></span>
