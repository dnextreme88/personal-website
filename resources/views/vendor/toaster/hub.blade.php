<div
    role="status"
    id="toaster"
    x-data="toasterHub(@js($toasts), @js($config))" @class([
...
>
    <template x-for="toast in toasts" :key="toast.id">
        <div x-show="toast.isVisible"
            ...
            :class="toast.select({ error: 'text-gray-200', info: 'text-gray-800', success: 'text-gray-200', warning: 'text-gray-200' })"
        >
            ...
            @if ($closeable)
                <button @click="toast.dispose()" aria-label="@lang('close')" class="absolute right-0 p-2 focus:outline-none hover:cursor-pointer rtl:right-auto rtl:left-0 {{ $alignment->is('bottom') ? 'top-3' : 'top-0' }}">
